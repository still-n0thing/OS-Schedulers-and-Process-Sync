...
void printall(Data dt[], long long n){
    printf("PID\tAT\tBT\tCT\tTOT\tWT\tRT\n");
    for(long long i = 0; i < n; i++){
        printf("%2lld\t%2lld\t%2lld\t%2lld\t%2lld\t%2lld\t%2lld\n",
        dt[i].pid, dt[i].at, dt[i].bt, dt[i].ct, dt[i].tot, dt[i].wt, dt[i].wt
        );
        fflush(stdout);
    }
}
 
void update(Data dt[], long long n, long long cst){
    long long cur = 0;
    for(long long i = 0; i < n; i++){
        dt[i].bt += cst; // For context switching time
        if(dt[i].at >= cur){
...
void solve(long long tc){
    ...
    long long cst = 1; // Context Switching Time
    insertion_sort(a, n);
    update(a, n, cst);
    ...
    printf("Context Switching Time = %lf\n", (double)cst);
    printf("\n");
}
