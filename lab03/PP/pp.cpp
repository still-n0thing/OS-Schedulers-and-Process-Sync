...
public:
    ...
    void set(int pid, int at, int bt, int priority){
        this->pid = pid;
        this->at = at;
        this->bt = bt;
        this->st = -1;
...
void printall(const vector<Process> & p){
    ...
    for(int i = 0; i < n; i++){
        ...
        printElement(p[i].priority, width);
...
void solve(){
    ...
    vector<Process> p(n);
    for(int i = 0; i < n; i++){
        int pid, at, bt, priority; cin >> pid >> at >> bt >> priority;
        p[i].set(pid, at, bt, priority);
    } // O(n)
